...
struct MockVersionedPortfileProvider : PortFileProvider::IVersionedPortfileProvider
{
    ...
    const std::vector<vcpkg::Versions::VersionSpec>& get_port_versions(
            const std::string& port_spec) const
    {
        (void)port_spec;
        static std::vector<vcpkg::Versions::VersionSpec> port_versions {  };
    ...
    {
        return get_control_file(version_spec.port_name, version_spec.version);
    }
